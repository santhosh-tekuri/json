...
func (d *details) DecodeJSON(de json.Decoder) error {
	return json.DecodeObj("details", de, func(de json.Decoder, prop json.Token) (err error) {
		switch {
		case prop.Eq("height"):
...
func ExampleDecodeJSON() {
	...
	emp := employee{}
	if err := emp.DecodeJSON(de); err != nil {
		panic(err)
	}
	fmt.Printf("%#v\n", emp)
	// Output:
